...
public class StockClosingConditionOfTwseDownloader_RunnableDownloaderTest {
	...
	public void beforeClass() throws IOException {
		...
		runnableDownloader = applicationContext.getBean(
				RunnableDownloader.class, beginDate, endDate);
	...
	public void moveToTargetPage() {
		...
		String breadcrumbs = browser.getDiv(By.cssSelector("#breadcrumbs"))
				.getText();
	...
	private Set<Runnable> generateRunnables(int runnableAmout) {
		...
		for (int i = 0; i < runnableAmout; ++i) {
			...
			Runnable runnable = applicationContext.getBean(
					RunnableDownloader.class, beginDate, endDate);
